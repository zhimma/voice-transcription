...
sealed class PrecompiledBinary implements SqliteBinary {
  ...
  Future<File> downloadIntoOutputDirectoryShared(
    ...
    // Prefer repository-prebuilt binaries to avoid flaky network downloads.
    final sourceFilename = library.sourceFilename;
    final localPrebuilt = File(
      p.join(input.packageRoot.toFilePath(), 'prebuilt', sourceFilename),
    );
    if (localPrebuilt.existsSync()) {
      localPrebuilt.copySync(downloadedFile.path);
    ...
    // If the download directory changes between runs, try to reuse an existing
    // cached binary from any previous download-* directory before fetching.
    final sharedDir = Directory(
      input.outputDirectoryShared.toFilePath(),
    );
    if (sharedDir.existsSync()) {
      final candidates = sharedDir
          .listSync()
          .whereType<Directory>()
          .where((d) => p.basename(d.path).startsWith('download-'));
      for (final candidate in candidates) {
        final candidateFile = File(p.join(candidate.path, fileName));
        if (!candidateFile.existsSync()) continue;
        final actualHash =
            await candidateFile.openRead().transform(sha256).first;
        if (actualHash.toString() == expectedHash) {
          candidateFile.copySync(downloadedFile.path);
          return downloadedFile;
        }
      }
    }
